...
func New(movies []*models.Movie) *Graph {
	// first, we describe the kinds of things we want to expose from our DB
	// we have to declare fields in here, which must match DB field names
	var movieType = graphql.NewObject(
		graphql.ObjectConfig{
			Name: "Movie",
			Fields: graphql.Fields{
				// provide the fields here (must exactly match your DB fields)
				"id": &graphql.Field{
					Type: graphql.Int,
				},
				"title": &graphql.Field{
	...
	// Fields represent actions you wish to perform on the 'movieType' variable
	// define the kinds of ways you will use the data (for this we use graphql.Fields{})
